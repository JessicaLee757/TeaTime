...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadActiveSession = async () => {
      try {
        const { data } = await supabase.from('sessions').select('*').eq('is_active', true).order('id', { ascending: false }).maybeSingle();
        if (data) {
          setConfig({
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      if (data) {
        setOrders(data.map((o: any) => ({ ...o, memberName: o.member_name, itemName: o.item_name })));
      }
    };
    fetchOrders();
  }, []);
  ...
  const handleStartSession = async (newConfig: SessionConfig) => {
    try {
      const { error } = await supabase.from('sessions').insert([{ shop_name: newConfig.drinkShopName, menu_data: newConfig.drinkItems, is_active: true }]);
  ...
  const handleEndSession = async () => {
    if (!window.confirm("確定要結束本次團購並清除所有訂單嗎？")) return;
    try {
  ...
  const handleOrderSubmit = async (newOrder: any) => {
    try {
      const { error } = await supabase.from('orders').insert([{ member_name: newOrder.memberName || newOrder.member_name, item_name: newOrder.itemName || newOrder.item_name, price: newOrder.price, notes: newOrder.notes }]);
      if (error) throw error;
      alert('點餐成功！');
      ...
      <div className="max-w-4xl mx-auto">
        <header className="flex justify-between items-center mb-12">
          <div className="flex items-center gap-3">
            <div className="bg-orange-500 p-2 rounded-lg text-white"><IconComponents.Coffee size={24} /></div>
            <h1 className="text-xl font-bold text-orange-600 tracking-tight text-orange-600">TeaTime</h1>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={handleReset} className="p-2 text-gray-400 hover:text-orange-500"><IconComponents.Users size={20} /></button>
            {!isParticipantLink && (
              <div className="flex items-center gap-3 bg-white border px-3 py-1.5 rounded-full shadow-sm ml-2">
                <select value={role} onChange={(e) => setRole(e.target.value as Role)} className="text-sm font-medium text-gray-700 bg-transparent focus:outline-none cursor-pointer">
